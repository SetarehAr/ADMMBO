...
function [mu,sigma2,hyp] = get_posterior(X,y,x_hats,opt,hyp)
    ...
    if isnumeric(hyp)
        ...
        hyp = [];
        %% ADDED BY SETAREH ARIAFAR WHEN USING SHIFTED MEAN Function
        
        hyp.mean = zeros(n_mh,1);
        if length(func2str(meanfunc{1}))==11
            if (func2str(meanfunc{1})=='meanShifted')
                hyp.mean =[opt.y,opt.z,opt.rho];
            end
        end
        if length(func2str(meanfunc{1}))==14
            if (func2str(meanfunc{1})=='meanShiftedBar')
                hyp.mean =[mean(cat(3,opt.y{:}),3),mean(cat(3,opt.z{:}),3),opt.rho];
            end
        end
        if length(func2str(meanfunc{1}))==17 || length(func2str(meanfunc{1}))==18
            if (func2str(meanfunc{1})=='meanShiftedBar_4d') || (func2str(meanfunc{1})=='meanShiftedBar_11d')
                hyp.mean =[mean(cat(3,opt.y{:}),3),mean(cat(3,opt.z{:}),3),opt.rho];
            end
        end
...
function [eic,eic_grad] = eic_obj_grad(z,X,Qiys,Qiks,cov_grad_f,best,hyps,opt)
	if nargout > 1,
		[ei,ei_grad] = ei_obj_grad(z,X,Qiys{1},Qiks{1},cov_grad_f,best, {1},opt);
	else
