function [minsample,minvalue,botrace] = bayesopt_SingleConst_CoupledFC(F,optt,Problem)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Added for ADMMBO
    opt=optt.f;
    funcfc=str2func(Problem.fc);
    FC=@(X) funcfc(X,Problem.f_parameters,Problem.c_parameters);
    PenTerm=@(X)(optt.ADMM.rho/2)*norm(X-(optt.f.z)+(optt.f.y/optt.ADMM.rho))^2;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    warning('off')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Adjusted for ADMMBO
                     f_vali=opt.initial_values(i);
                     vali=opt.initial_values(i)+PenTerm(init_pt);
                     c_vali=opt.initial_con_values(i);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Adjusted for ADMMBO  
				...
				values = [values;vali];% Note that this is the AL_X(.) value not F(.)
                con_values =[con_values;c_vali];
                f_values=[ f_values;f_vali];
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%              
				if DO_CBO,
					con_values = [con_values;coni];
				end
			end	
        else %changed instead of end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Adjusted for ADMMBO
                [f_value,c_value]= FC(hyper_cand);
                value=f_value+PenTerm(hyper_cand);
        		times(end+1) = toc;
		        samples = [samples;scale_point(hyper_cand,opt.mins,opt.maxes)];
		        values(end+1) = value;
                f_values(end+1) = f_value;
                con_values(end+1) = c_value;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function [mu,sigma2,hyp] = get_posterior(X,y,x_hats,opt,hyp)
    ...
    if isnumeric(hyp)
        ...
        hyp = [];
        %% ADDED BY SETAREH ARIAFAR WHEN USING SHIFTED MEAN
        
        if (func2str(meanfunc{1})=='meanShifted7d')
        % if (func2str(meanfunc{1})=='meanShifted'
       %hyp.mean = zeros(n_mh,1);
        hyp.mean =[opt.y,opt.z,opt.rho];
        end
      %  if (func2str(meanfunc{1})=='meanShifted4d')
       % hyp.mean =[opt.y,opt.z,opt.rho];
        %end
        %hyp.cov = zeros(n_ch,1);
        ell = 1/4; sf = 1; hyp.cov = log([ell; sf]); %changed at Jan 22 for matern 5 kernel
        hyp.lik = log(0.1);
