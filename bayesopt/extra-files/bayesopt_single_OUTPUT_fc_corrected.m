function [minsample,minvalue,botrace] = bayesopt_single_OUTPUT_fc_corrected(F,opt,Problem)
	...
	if isfield(opt,'resume_trace') && opt.resume_trace && exist(opt.trace_file,'file')
        ...
        con_values = botrace.con_values;%%NEW%%
        %
		times = botrace.times;
		if DO_CBO,
		...
		if isfield(opt,'initial_points'),
			...
			for i = 1:size(opt.initial_points,1),
				...
				if ~DO_CBO,
				%vali = F(init_pt);
                   vali=opt.initial_values(i);%%NEW%%
                   coni=opt.initial_con_values(i);%%NEW%%
				else
					[vali,coni] = F(init_pt);
				end
    ...
    for i = i_start:opt.max_iters-2,
        ...
        botrace.con_values = con_values;%%NEW%%
        botrace.times = times;
		if DO_CBO,
