function [minsample,minvalue,botrace] = bayesopt_single_FC(F,opt,Problem)
	...
	if isfield(opt,'resume_trace') && opt.resume_trace && exist(opt.trace_file,'file')
		...
		if isfield(opt,'initial_points'),
			...
			for i = 1:size(opt.initial_points,1),
				...
				if ~DO_CBO,
					%vali = F(init_pt);
                   % [vali,coni] = FC(init_pt);
                   vali=opt.initial_values(i);
                   coni=opt.initial_con_values(i);
                   %[vali,coni]=[opt.initial_values(i) ,opt.initial_con_values(i)];
				else
					[vali,coni] = F(init_pt);
				end
				samples = [samples;sinit_pt];
				values = [values;vali];
                con_values = [con_values;coni];%new
				if DO_CBO,
	    ...
	    if ~DO_CBO,
          ...
          %  [val1,con1] = FC(pt1);
              val1=opt.initial_values(1);
              con1=opt.initial_con_values(1);
		else
			[val1,con1] = F(pt1);
		end
	    ...
	    if ~DO_CBO,
           ...
           % [val2,con2] = FC(pt2);
              val2=opt.initial_values(2);
              con2=opt.initial_con_values(2);
		else
			[val2,con2] = F(pt2);
        ...
        con_values = [con_values;con1;con2];%new
		if DO_CBO,
    ...
    for i = i_start:opt.max_iters-2,
		...
		if ~DO_CBO,
			if PAR_JOBS <= 1,
	        	tic;
				%value = F(hyper_cand);
                [value con_value]=FC(hyper_cand)
        		times(end+1) = toc;
		        samples = [samples;scale_point(hyper_cand,opt.mins,opt.maxes)];
		        values(end+1) = value;
                con_values(end+1)=con_value; %new
			else
				par_values = {};
				par_times = {};
				parfor k = 1:PAR_JOBS,
        ...
        botrace.con_values = con_values; %new constraints values added
        botrace.times = times;
		if DO_CBO,
