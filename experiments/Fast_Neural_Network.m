...
bounds=[0 1;0 1;0 1;-4 1;0 100;-4 0;-3 0;50 500;50 500;50 500;0 1]; % Bounds (Upper and Lower) for the variables- a matrix with dimensions "DimNum * 2 "
X0=bounds(:,1); % to keep consistant with Spearmint implementation
X0=X0';


initial_bud_c=20;
dim = size(bounds,1);
InfPoint=(bounds(:,1)'-1000); % An infeasible point outside the bounds
Inf_obj_Value=2000;

%% Defining a struct called "problem" for setting the objective and constraint functions & bounds
problem.f='acc_eval';
problem.f_parameters=[];
problem.c={'time_eval'}; % a struct to enable multiple constraints
problem.c_parameters={[]};
problem.bounds=bounds;
problem.InfPoint=(bounds(:,1)'+1000);
total_budget=100*(length(problem.c)+1);
Iters = 1+ round((total_budget - (initial_bud_c*(length(problem.c))) - initial_bud_f)/((length(problem.c)+1) *final_bud));
...
opt.f.reduced_step_iters=final_bud;% 
opt.f.meanfunc = {@meanShiftedBar_11d}; % Constant mean function.
...
FileName='FastNN_results_ADMMBO.mat';
