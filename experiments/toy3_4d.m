...
problem.f='Sum_Linear4';
problem.f_parameters=[];
problem.c={'C_4d'}; % a struct to enable multiple constraints
...
F = @(X) funcf(X(1),X(2),X(3),X(4),problem.f_parameters);


C = cell(length(problem.c),1);
for i=1:length(problem.c)
    func=str2func(problem.c{i});
    C{i}= @(X) func(X(1),X(2),X(3),X(4),problem.c_parameters{i});
    clear func
end
...
opt.f.meanfunc = {@meanShiftedBar_4d}; 
...
%opt.f.num_cov_hypers=5;
opt.f.num_mean_hypers=9;
opt.f.rho=rho;
%%%%%%%%%%%%%%%%%%%%%%%%    
...
FileName='toy3_results_ADMMBO.mat';
