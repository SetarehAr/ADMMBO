function [ret_minval,final_xatmin,history] =Direct_with_INPUT_Parameters_single...
...
getopts(opts, ...
 'maxits',     1000,...         % maximum of iterations
        ...
        for i = 1:Problem.numconstraints
            if length(Problem.constraint(i).func) == length(Problem.f)
                if double(Problem.constraint(i).func) == double(Problem.f)                    
                    ...
                    con_value = feval(Problem.constraint(i).func,point(1),point(2),[Problem.constraint(i).parameters],varargin{:});
                end
            else
                con_value = feval(Problem.constraint(i).func,point(1),point(2),[Problem.constraint(i).parameters],varargin{:});
            end
...
if calltype == 1
    ...
    fcn_value = feval(Problem.f,point(1),point(2),[Problem.fparameters],varargin{:});
elseif calltype == 2
    %-- f returns all constraints
    [fcn_value, cons] = feval(Problem.f,point(1),point(2),[Problem.fparameters],varargin{:});
    for i = 1:length(cons)
        if cons > 0
...
elseif calltype == 4
    %-- f returns feas flag
    [fcn_value,feas_flag] = feval(Problem.f,point(1),point(2),[Problem.fparameters],varargin{:});
elseif calltype == 5
